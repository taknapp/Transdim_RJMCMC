...
class PopulationOmegaGW(object):
    ...
    def draw_mass_proposal_samples(self, population_params, N, grids = MASS_GRIDS):
        ...
        mass_proposal_samples =  {'mass_1_source': np.array(samples['mass_1'])[:N], self.other_mass_coord: np.array(samples[self.other_mass_coord])[:N]}
    ...
    def calculate_weights(self, Lambda=None):
        ...
        if Lambda is not None:
            ...
            if self.backend == 'jax':
                self.weights.at[np.where(probabilities==0.0)].set(0.0)
            else:
